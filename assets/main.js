...
(function () {
  ...
  function renderGoRank() {
    ...
    $('gorank-note').textContent = '各订阅计划折算后的单价，按折算 token 单价升序（单价 = 0.95×(命中×缓存读 + (1−命中)×输入) + 0.05×输出，输入占比 95%、命中率用上方滑块调）。OpenCode Go：折后 = 表列价 × 10/额度，DeepSeek 按非高峰档；智谱 Coding Plan v2：实测周额度按 Lite 的 5×（Pro）/20×（Max）放大（GLM-5.3 50M/周、Flash 125M/周，高峰约 1/3，额度不分输入输出缓存）÷ 月费；v3（Lite ¥118/Pro ¥538/Max ¥1078 每月）：按官方积分系数折算（GLM-5.3 6.9/1.7/24、Flash 2.3/0.56/8，非高峰 5 折），积分额度按 Lite 的 5×/20×。官方 API 牌价作为独立条目列入同一排序（每个模型只列一次），与套餐条目同公式，便于直接对比。';
  }
