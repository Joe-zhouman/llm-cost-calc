...
(function () {
  ...
  var state = { R: 0.5, gx: 1, tpHit: 0.9, killBase: 'dsv4flash', gorankPlan: '', gorankModel: '' };
  try {
    ...
    if (saved) {
      ...
      if (saved && typeof saved === 'object') {
        ...
        if (typeof saved.gorankPlan === 'string') state.gorankPlan = saved.gorankPlan;
        if (typeof saved.gorankModel === 'string') state.gorankModel = saved.gorankModel;
      }
    }
  ...
  function initGoRank() {
    ...
    // 筛选器：按套餐 / 按模型（模型按去括号裸名匹配，合并 max/高峰/限时 等档位行）
    var planSel = $('gorank-plan');
    var modelSel = $('gorank-model');
    if (planSel && modelSel) {
      var plansAll = D.plans || [];
      var planNames = [], modelNames = [], seenPlan = {}, seenModel = {};
      for (var i = 0; i < plansAll.length; i++) {
        var pn = plansAll[i].plan;
        if (!seenPlan[pn]) { seenPlan[pn] = 1; planNames.push(pn); }
        var mb = bareName(plansAll[i].name);
        if (!seenModel[mb]) { seenModel[mb] = 1; modelNames.push(mb); }
      }
      modelNames.sort();
      var html = '<option value="">全部</option>';
      for (var j = 0; j < planNames.length; j++) html += '<option value="' + planNames[j] + '">' + planNames[j] + '</option>';
      planSel.innerHTML = html;
      planSel.value = state.gorankPlan;
      if (planSel.value !== state.gorankPlan) state.gorankPlan = ''; // 存的套餐已不存在→重置
      html = '<option value="">全部</option>';
      for (var k = 0; k < modelNames.length; k++) html += '<option value="' + modelNames[k] + '">' + modelNames[k] + '</option>';
      modelSel.innerHTML = html;
      modelSel.value = state.gorankModel;
      if (modelSel.value !== state.gorankModel) state.gorankModel = '';
      planSel.addEventListener('change', function () {
        state.gorankPlan = this.value;
        persist();
        renderGoRank();
      });
      modelSel.addEventListener('change', function () {
        state.gorankModel = this.value;
        persist();
        renderGoRank();
      });
    }
  ...
  function renderGoRank() {
    ...
    // 筛选：套餐精确匹配；模型按去括号裸名合并档位行（含官方牌价条目）
    var planFilter = state.gorankPlan || '';
    var modelFilter = state.gorankModel || '';
    if (planFilter || modelFilter) {
      var kept = [];
      for (var f = 0; f < rows.length; f++) {
        if (planFilter && rows[f].plan !== planFilter) continue;
        if (modelFilter && bareName(rows[f].name) !== modelFilter) continue;
        kept.push(rows[f]);
      }
      rows = kept;
    }
